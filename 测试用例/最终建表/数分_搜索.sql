            ...
            (
                ...
                from
                    (
                        ...
                        WHERE dt between '${zdt.addDay(-1).format("yyyyMMdd")}'-- 自定义日期
...
select a.datee
     ...
     ,concat(round(a.click_item/a.show_item*100,2),'%') as `搜索点击率_item`
     ,concat(round(a.order_num/a.show_item*100,2),'%') as `搜索预定率_item`
     ,concat(round(a.top3_click_pv/a.click_pv*100,2),'%') as `TOP3点击命中率_PV`
     ,concat(round(a.top3_order_pv/a.order_pv*100,2),'%') as `TOP3预定命中率_PV`
     ,concat(round(a.medal_show_item/a.show_item*100,2),'%') as `挂牌酒店流量占比`
--      ,concat(round(a.no_price_show_item/a.show_item*100,2),'%')  as `无库存流量占比`
     ,concat(round(a.no_price_show_item/a.show_item*100,2),'%')  as `无库存流量占比`
     ,concat(round((a.sort_right_show_item/a.show_item)*100,2),'%')  as `render与sort价格一致率`
     ...
     ,concat(round(a.order_num/a.show_uv*100,2),'%') as `CR`
     ,concat(round((a.show_adr/b.order_adr-1)*100,2),'%')  as `曝光与订单adr_gap`
     ,cast(a.avg_click_rank as decimal(20,2)) as `平均点击深度_PV`
     ,cast(a.avg_order_rank as decimal(20,2)) as `平均预定深度_PV`
     ,concat(round(a.cover_rate_show_pv_3*100,2),'%')  as `Top3产品力覆盖率`
     ,concat(round(a.lose_rate_show_pv_3*100,2),'%') as `Top3产品力lose率`
     ,cast(b.order_adr as decimal(20,2)) as `订单ADR`
     ,cast(b.total_room_night/a.order_uv as decimal(20,2)) as `人均间夜量`
     ,concat(cast(b.total_order_commission/b.total_gmv*100 as decimal(20,2)),'%') as `佣金率`
from
    search_result a
        join order_result b
             on a.datee = b.datee
